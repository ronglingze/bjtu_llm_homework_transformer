...
为了节省下载时间和确保数据完整性，项目提供了预打包的数据压缩文件 `data.tar.gz`：
...
├── iwslt2017/                                          # HuggingFace数据集
│   └── iwslt2017-en-de/
│       └── 1.0.0/
│           └── [dataset_hash]/
│               ├── iwslt2017-train.arrow              # 训练集数据
│               ├── iwslt2017-validation.arrow         # 验证集数据
│               ├── iwslt2017-test.arrow               # 测试集数据
...
└── downloads/                                          # 原始下载文件
    └── extracted/
        └── [extract_hash]/
            └── en-de/
                ├── train.tags.en-de.en                # 训练集英文
                ├── train.tags.en-de.de                # 训练集德文
                ├── IWSLT17.TED.dev2010.en-de.*.xml    # 验证集XML文件
                ├── IWSLT17.TED.tst*.en-de.*.xml       # 测试集XML文件
                └── README                              # 数据集说明
```

#### 方式二：自动下载

如果不使用预打包数据，程序会在首次运行时自动下载IWSLT2017数据集（约500MB）到指定的 `data_dir` 目录中。

**数据格式说明：**
- `*.arrow` 文件是HuggingFace Datasets的二进制格式，包含预处理后的数据
- `*.xml` 文件是原始IWSLT2017数据格式，用于参考和调试
- `tokenizer-*.json` 是预训练的WordPiece分词器文件
- 解压后的数据可以直接被项目的数据处理代码读取，无需额外处理

**注意：** 使用预打包数据可以显著减少数据准备时间，避免网络下载可能遇到的问题。
...
1. **数据准备**：项目提供了 `data.tar.gz` 预打包数据文件，建议解压使用以节省下载时间
2. 如果不使用预打包数据，首次运行时会自动下载数据集，需要稳定的网络连接
3. 建议使用GPU进行训练以获得更好的性能
4. 可以通过调整配置文件中的参数来优化模型性能
5. 混合精度训练可以减少显存使用并加速训练
6. 使用 `--dummy_data` 参数可以快速测试训练流程，无需数据集
7. 训练过程中的详细日志会保存到 `training.log` 文件中
8. 分词器文件会自动保存在 `data/` 目录中，下次运行时直接加载
9. 使用翻译功能前，请确保模型训练已完成且模型文件存在
10. 翻译质量取决于训练数据的质量和训练轮数，建议使用完整的训练周期
